...
for iter_num in MCiters:
    savename = ('sic_'+prior_name+'_anrecon_revisions1_'+str(recon_start)+'_'+
    ...
    for yk, target_year in enumerate(recon_years):
        ...
        if 'full' in cfg.prior.state_variables['sic_sfc_OImon']:
            cutoff_count_pos = cutoff_count_pos + (sic_lalo[grid.lat[:,0]>0,:,:]>100.0).sum()
            cutoff_count_neg = cutoff_count_neg + (sic_lalo[grid.lat[:,0]>0,:,:]<0.0).sum()
