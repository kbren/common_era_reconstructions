...
for t,it in enumerate(iteration):
    ...
    savedir = ('/home/disk/p/mkb22/nobackup/LMR_output/reanalysis_reconstruction_data/'+
...
#  #               'sic_'+prior_name+'_annual_recon_1900_1940_cru_R0_4_10deg_inf'+inf_name+'_loc'+str(loc_list[1])+it+'.pkl']
    ...
    savename = ['sic_'+prior_name+'_annual_recon_1850_2018_cru_gis_be_R0_4_10deg_inf'+inf_name+'_loc'+str(loc_list[0])+it+'.pkl',
                'sic_'+prior_name+'_annual_recon_1850_2018_cru_gis_be_R0_4_10deg_inf'+inf_name+'_loc'+str(loc_list[0])+it+'.pkl',
                'sic_'+prior_name+'_annual_recon_1850_2018_cru_gis_be_R0_4_10deg_inf'+inf_name+'_loc'+str(loc_list[0])+it+'.pkl',
                'sic_'+prior_name+'_annual_recon_1850_2018_cru_gis_be_R0_4_10deg_inf'+inf_name+'_loc'+str(loc_list[0])+it+'.pkl',
                'sic_'+prior_name+'_annual_recon_1850_2018_cru_gis_be_R0_4_10deg_inf'+inf_name+'_loc'+str(loc_list[0])+it+'.pkl']#,
#                 'sic_'+prior_name+'_annual_recon_1979_2018_cru_R0_4_10deg_inf'+inf_name+'_loc5000'+it+'.pkl']
    ...
    #     print('working on localization radius: ',cfg.core.loc_rad)

        # option to change the localization radius in the config file (used by DA code)
        print(cfg.core.loc_rad)
        cfg_params = LMR_utils.param_cfg_update('core.loc_rad',loc)
        cfg_new = LMR_config_greg.Config(**cfg_params)
        ...
        #    R = Rvar
            ...
            #recon_years = range(1979,max(analysis_time[ref_dset]))
            #recon_years = range(1900,1941)
    ...
    #             Ye_QC = np.squeeze(Ye_QC1[qcpass_R])
                ...
                xam,Xap = LMRlite.Kalman_ESRF(cfg_new,obs_QC,R_QC,Ye_QC,
    ...
    #        sic_dict[ref_dset] = sic_save
            ...
            sic_ens_dict[ref_dset] = np.squeeze(np.array(sic_full_ens))*nharea
            sie_ens_dict[ref_dset] = np.squeeze(np.array(sie_full_ens))*nharea
        ...
        sic_recon['sia ens dict'] = sic_ens_dict
        sic_recon['sie ens dict'] = sie_ens_dict
        sic_recon['recon_years_all'] = recon_years_all
        sic_recon['R_all'] = R_QC
        sic_recon['Ye_full'] = Ye_full 

        print('Saving to: ',savedir+savename[i])
        pickle.dump(sic_recon,open(savedir+savename[i], "wb"))
