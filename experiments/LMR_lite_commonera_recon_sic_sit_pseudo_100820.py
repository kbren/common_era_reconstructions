...
#cfile = '/home/disk/kalman2/mkb22/LMR_lite/configs/config_mpi_sit_brennan2020.yml'
...
#    sit_pos = X_regrid.trunc_state_info['sit_noMV_OImon']['pos']
...
#    sic_prior_regrid_true = sic_prior_regrid[recon_ind_start:recon_ind_end,:,:]
    ...
    #Xb_one_sic = Xb_one_new[sic_pos[0]:sic_pos[1]+1,:]
...
#    sit_full_Sens = []
    ...
    #        print('assimilating ob '+str(i))
            if pp_err is False: 
                p_err = prox_err[target_year][iob]*ob_factor
            else: 
                p_err = pp_err
...
#                               [grid.nlat,grid.nlon,grid.nens])
        ...
        for k in range(grid.nens):
            gmtas,nhtas,shtas= lmr.global_hemispheric_means(tas_lalo[:,:,k],grid.lat[:, 0])
...
#            _,nhmit,shmit= lmr.global_hemispheric_means(sit_lalo[:,:,k],grid.lat[:, 0])
            ...
            tas_Gens.append(gmtas)
            tas_Nens.append(nhtas)
...
#        sit_save_lalo[yk,:,:] = np.nanmean(sit_lalo,axis=2)
        ...
        tas_full_Gens.append(tas_Gens)
        tas_full_Nens.append(tas_Nens)
...
#    sic_recon['sit_lalo'] = sit_save_lalo
    ...
    sic_recon['tas_Gens'] = np.squeeze(np.array(tas_full_Gens))
    sic_recon['tas_Nens'] = np.squeeze(np.array(tas_full_Nens))
