...
sys.path.insert(1,'')
import common_recons_utils as rrutils
...
#cfg = None
cfg = LMRlite.load_config(cfile)[0]
...
regional_masks = rrutils.load_regional_masks()
        
# END USER PARAMETERS: 
#--------------------------------------------------------------------

for iter_num in MCiters:
    savename = ('sic_'+prior_name+'_anrecon_revisions2_regions_testcount2_'+str(recon_start)+'_'+
    ...
    p_1 = np.zeros(nyears)
    p_5 = np.zeros(nyears)
    p_10 = np.zeros(nyears)

    plow_1 = np.zeros(nyears)
    plow_5 = np.zeros(nyears)
    plow_10 = np.zeros(nyears)

    p100 = np.zeros(nyears)
    p101 = np.zeros(nyears)
    p105 = np.zeros(nyears)
    p110 = np.zeros(nyears)

    phigh101 = np.zeros(nyears)
    phigh105 = np.zeros(nyears)
    phigh110 = np.zeros(nyears)
    
    [reg_sie_anom,reg_sie_lowanom,reg_sie_highanom,
     reg_sic_anom,reg_sic_lowanom,reg_sic_highanom] = rrutils.initialize_regional_dict(regions,nyears,grid.nens)
    ...
    for yk, target_year in enumerate(recon_years):
        ...
        if 'full' in cfg.prior.state_variables['sic_sfc_OImon']:
            ...
            sic_nh_og = sic_lalo_og[grid.lat[:,0]>0,:,:]
            
            sic_nh_og_1d = sic_nh_og.flatten()
            
            p0[yk] = stats.percentileofscore(sic_nh_og_1d,0)
            p_1[yk] = stats.percentileofscore(sic_nh_og_1d,-1)
            p_5[yk] = stats.percentileofscore(sic_nh_og_1d,-5)
            p_10[yk] = stats.percentileofscore(sic_nh_og_1d,-10)
            
            plow_1[yk] = stats.percentileofscore(sic_nh_og[(sic_nh_og<0)],-1)
            plow_5[yk] = stats.percentileofscore(sic_nh_og[(sic_nh_og<0)],-5)
            plow_10[yk] = stats.percentileofscore(sic_nh_og[(sic_nh_og<0)],-10)
            
            p100[yk] = stats.percentileofscore(sic_nh_og_1d,100)
            p101[yk] = stats.percentileofscore(sic_nh_og_1d,101)
            p105[yk] = stats.percentileofscore(sic_nh_og_1d,105)
            p110[yk] = stats.percentileofscore(sic_nh_og_1d,110)
            
            phigh101[yk] = stats.percentileofscore(sic_nh_og[(sic_nh_og>100)],101)
            phigh105[yk] = stats.percentileofscore(sic_nh_og[(sic_nh_og>100)],105)
            phigh110[yk] = stats.percentileofscore(sic_nh_og[(sic_nh_og>100)],110)
        ...
        for k in range(grid.nens):
            ...
            for reg in regions:
                [reg_sie_anom[reg][yk,k],reg_sie_lowanom[reg][yk,k],
                reg_sie_highanom[reg][yk,k],reg_sic_anom[reg][yk,k],
                reg_sic_lowanom[reg][yk,k],
                reg_sic_highanom[reg][yk,k]] = rrutils.calc_regional_totals(np.moveaxis(sic_lalo,2,0), 
                                                                            np.moveaxis(sie_lalo,2,0), 
                                                                            regional_masks[reg].values)
        
        sic_save_lalo_og[yk,:,:,:] = sic_lalo_og
    ...
    percentiles['p_1'] = p_1
    percentiles['p_5'] = p_5
    percentiles['p_10'] = p_10

    percentiles['plow_1'] = plow_1
    percentiles['plow_5'] = plow_5
    percentiles['plow_10'] = plow_10

    percentiles['p100'] = p100
    percentiles['p101'] = p101
    percentiles['p105'] = p105
    percentiles['p110'] = p110

    percentiles['phigh101'] = phigh101
    percentiles['phigh105'] = phigh105
    percentiles['phigh110'] = phigh110
    
    regional_sie = {}
    regional_sia = {}
    
    regional_sie['reg_sie_anom'] = reg_sie_anom
    regional_sie['reg_sie_lowanom'] = reg_sie_lowanom
    regional_sie['reg_sie_highanom'] = reg_sie_highanom
    regional_sia['reg_sia_anom'] = reg_sic_anom
    regional_sia['reg_sic_lowanom'] = reg_sic_lowanom
    regional_sia['reg_sic_highanom'] = reg_sic_highanom
    
    sic_recon = {}
    
    sic_recon['percentiles'] = percentiles
    ...
    sic_recon['regional_sie'] = regional_sie
    sic_recon['regional_sia'] = regional_sia
